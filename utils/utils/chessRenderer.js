  ...
  for (let row = 0; row < 8; row++) {
    for (let col = 0; col < 8; col++) {
      ...
      if (piece) {
        ...
        const cx = x + SQUARE / 2;
        const cy = y + SQUARE / 2 + 4;
        // Use the filled (black) glyph variants for BOTH colors and control
        // fill/stroke ourselves — guarantees strong contrast regardless of
        // which font a given renderer substitutes for the outline glyphs.
        const solidGlyph = PIECE_GLYPH[piece.toLowerCase()];
        piecesXml +=
          `<text x="${cx}" y="${cy}" font-size="${SQUARE * 0.72}" ` +
          `text-anchor="middle" dominant-baseline="middle" ` +
          `fill="${isWhitePiece ? "#fefefe" : "#161512"}" ` +
          `stroke="${isWhitePiece ? "#161512" : "none"}" stroke-width="${isWhitePiece ? 2.5 : 0}" ` +
